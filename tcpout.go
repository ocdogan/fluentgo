...
func newTCPOut(manager InOutManager, config *inOutConfig) *tcpOut {
	if config == nil {
		return nil
	}
	...
	if f, ok = params["connTimeoutSec"].(float64); ok {
		connTimeoutSec = minInt(30, maxInt(0, int(f)))
	}
			...
			if msg != "" {
				err = func() error {
					...
					if conn != nil {
						...
						conn.Write([]byte(tcpUDPMsgStart))
